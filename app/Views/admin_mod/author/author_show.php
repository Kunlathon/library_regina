...
<?php
							...
							<table class="table table-hover table-bordered" id="datatable-basic-library_rc">
								<thead>
									<tr align="center">
										...
										<th align="center"><div>ชื่อผู้แต่ง</div></th>
	...
	<?php
			...
			foreach($ReadManagementAuthor->RunAuthor() as $rc=>$AuthorRow){ ?>
<!--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++-->	
									<tr>
										<td align="center"><div><?php echo @$AuthorRow["LA_Key"];?></div></td>
										<td><div><?php echo @$AuthorRow["LA_Name"];?></div></td>
										<td>
											<ul class="list-inline">
												<li>
													<button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#DeleteModal<?php echo @$AuthorRow["LA_Key"];?>"><i class="icon-bin2"></i></button>
												</li>
												<li>
<form name="form_update<?php echo @$AuthorRow["LA_Key"];?>" id="form_update<?php echo @$AuthorRow["LA_Key"];?>"  method="post" action="<?php echo base_url();?>admin?library_mod=author">													
													<button type="submit" class="btn btn-danger btn-xs"><i class="icon-pencil6"></i></button>
<input type="hidden" name="manage" id="manage" value="form_update"> <input type="hidden" name="author_key" id="author_key" value="<?php echo @$AuthorRow["LA_Key"];?>">
</form>													
												</li>												
											</ul>
										</td>
...
<!--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++-->			
<div id="DeleteModal<?php echo @$AuthorRow["LA_Key"];?>" class="modal fade" role="dialog">
  	<div class="modal-dialog">
<!-- Modal content-->
    <div class="modal-content">
      	<div class="modal-header">
        	<button type="button" class="close" data-dismiss="modal">&times;</button>
        	<h4 class="modal-title"> <i class="icon-trash"> </i> ยืนยันลบข้อมูล : <?php echo @$AuthorRow["LA_Name"];?></h4>
      	</div>
      	<div class="modal-body">
				<div class="row" >
					<div class="col-<?php $grid;?>-6"><button type="button"  name="delete_<?php echo @$AuthorRow["LA_Key"];?>" data-dismiss="modal" id="delete_<?php echo @$AuthorRow["LA_Key"];?>" value="<?php echo @$AuthorRow["LA_Key"];?>" class="btn btn-success">ลบข้อมูล</button></div>
...
<script>
		$(document).ready(function(){
			$('#delete_<?php echo @$AuthorRow["LA_Key"];?>').on('click', function() {
				var manage="process_delete";
				var author_key=$("#delete_<?php echo @$AuthorRow["LA_Key"];?>").val();
					if(author_key!=""){
